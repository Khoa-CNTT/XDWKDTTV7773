...
export default function AuthModal({ isOpen, onClose, initialTab = "login", onForgotPassword }: AuthModalProps) {
  ...
  const handleLoginSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      setIsLoading(true);
      try {
        // Sử dụng NextAuth signIn để đăng nhập
        console.log('Đang thử đăng nhập với:', { email: loginEmail, password: loginPassword.substring(0, 1) + '***' });
        ...
        if (result?.error) {
          ...
          // Hàm lấy thông điệp cụ thể từ error code
          const getErrorMessage = (error: string) => {
            const errorMessages: Record<string, string> = {
              'CredentialsSignin': 'Thông tin đăng nhập không chính xác hoặc tài khoản không tồn tại',
              'Default': 'Đăng nhập thất bại, vui lòng kiểm tra lại thông tin'
            };
            
            // Nếu lỗi chứa thông điệp rõ ràng, sử dụng nó
            if (error.includes('Tài khoản') || error.includes('mật khẩu') || error.includes('Vui lòng')) {
              return error;
            }
            
            // Hiển thị lỗi gốc nếu không tìm thấy trong danh sách
            return errorMessages[error] || `Lỗi: ${error}`;
          };
        ...
        // Chuyển hướng đến trang profile/my-order sau khi đăng nhập thành công
        router.push('');
      } catch (err: any) {
  ...
  async (e: React.FormEvent) => {
    ...
    body: JSON.stringify({
    ho_ten: registerSurname + ' ' + registerName,
    email: registerEmail,
    password: registerPassword,
      ...
      router.push(`/verify?email=${registerEmail}`); // Chuyển đến trang xác minh OTP
    } catch (err: any) {
      toast.error(err.message || t("errorUnexpected"));
      console.error("Register error:", err);
    } finally {
      setIsLoading(false);
    }
  },
  [registerName, registerSurname, registerEmail, registerPassword, registerConfirmPassword, t, router]
);
